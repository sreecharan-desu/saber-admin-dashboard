...
export default function MyJobs() {
  ...
  const handleDelete = async (id: string) => {
    if (
      window.confirm("Delete this job? It will no longer appear to candidates.")
    ) {
      ...
      <div className="space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="h-[200px] card-base bg-white border-dashed border-gray-200"
            />
          ))}
        </div>
      </div>
    ...
    <div className="space-y-12">
      <div className="flex justify-between items-end">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1 font-normal">
            Active jobs published by your organization.
          </p>
        </div>
        <div className="flex gap-4 items-center">
          ...
          <Link
            to="/jobs/new"
            className="btn-primary h-11 px-6 flex items-center gap-2 text-[13px] font-bold tracking-tight"
          >
            <Plus size={16} />
            Post Job
          </Link>
      ...
      {jobs.length === 0 ? (
        <div className="flex flex-col items-center justify-center h-[40vh] text-center card-base py-12 px-8 border-dashed shadow-none">
          ...
          <Link to="/jobs/new" className="btn-secondary mt-8 h-10 px-8">
            Post First Job
          </Link>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {jobs.map((job: any, idx: number) => (
            <motion.div
              ...
              className={clsx(
                "card-base p-8 hover:border-black hover:shadow-md transition-all group bg-white",
                !job.active && "opacity-75 grayscale-[0.5]",
              )}
            >
                ...
                <div className="flex justify-between items-start mb-6">
                  ...
                  >
                    <div
                      className={clsx(
                        "dot shadow-sm",
                        job.active ? "bg-black animate-pulse" : "bg-gray-300",
                      )}
                    />
                    <span
                      className={clsx(
                        "text-[10px] font-bold uppercase tracking-widest",
                        job.active ? "text-black" : "text-gray-400",
                      )}
                    >
                ...
                <div className="mt-auto space-y-4">
                  <div className="flex flex-wrap gap-2">
                    {job.skills_required.slice(0, 3).map((skill: string) => (
                      <span
                        key={skill}
                        className="px-2 py-0.5 rounded bg-gray-50 border border-gray-200 text-[10px] text-gray-500 font-medium whitespace-nowrap"
                      >
                        {skill}
                      </span>
                    ))}
