...
export default function CreateJob() {
  ...
  const addSkill = () => {
    if (
      formData.newSkill &&
      !formData.skills_required.includes(formData.newSkill)
    ) {
      setFormData((prev) => ({
        ...prev,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const activeCompanyId =
      companyId || prompt("Please enter your Company ID:");
    ...
    try {
      ...
      const errorMsg =
        (err as ApiError).response?.data?.error || "Failed to save job";
      setError(errorMsg);
    } finally {
      setLoading(false);
    }
    ...
    <div className="max-w-3xl mx-auto py-8">
      <div className="mb-12">
        ...
        <p className="text-sm text-gray-500 mt-2 font-normal">
          {isEdit
            ? "Updating technical specifications for high-fidelity discovery."
            : "Define a technical problem, not a job description."}
        </p>
      </div>
        ...
        {!companyId && (
          <div className="bg-gray-50 rounded-md p-6 border border-black/10 flex flex-col gap-3">
            ...
            <p className="text-xs text-gray-500">
              Workspace ID could not be auto-detected. Please provide it to
              continue.
            </p>
          </div>
        )}
            ...
            <div className="space-y-8 text-left">
              <div className="relative">
                ...
                <textarea
                  ...
                  required
                />
                {formData.problem_statement.length > 0 &&
                  formData.problem_statement.length < 50 && (
            ...
            <div className="text-left">
              ...
              <div className="flex gap-3 mb-4">
                ...
                <button
                  type="button"
                  onClick={addSkill}
                  className="btn-primary w-12 h-10 p-0"
                >
                  <Plus size={20} />
                </button>
              </div>
              <div className="flex flex-wrap gap-2.5">
                {formData.skills_required.map((skill) => (
                  <span
                    key={skill}
                    className="inline-flex items-center gap-2 px-3 py-1.5 rounded-md text-xs font-bold bg-gray-100 text-black"
                  >
                    {skill}
                    <button
                      type="button"
              ...
              >
                ...
                <div className="text-left">
                  ...
                  <p className="text-xs text-gray-500 font-normal">
                    {formData.active
                      ? "This challenge is live and visible to all candidates."
                      : "This challenge is hidden and will not appear in discovery feeds."}
                  </p>
                </div>
              </button>
            </div>
